...
-   **Email Verification**: New user registration requires email verification before login. Verification tokens are SHA256-hashed for storage, with 24-hour expiry. Users see a "check your email" screen after registration. Unverified users receive a 403 on login with a `requiresVerification` flag. Resend verification endpoint available. Verify email page at `/verify-email?token=...`. Existing users were bulk-marked as verified during migration.
-   **Block User**: Users can block other users from the "..." menu on any post. Blocked users' posts are filtered from the feed. `userBlocks` table tracks blocks. API: `POST/DELETE /api/users/:userId/block`, `GET /api/users/:userId/is-blocked`. Developer is notified via server log on each block (Apple 1.2 compliance).
-   **Account Deletion**: Users can permanently delete their account from Settings → Delete Account. Requires typing "DELETE" to confirm. All user data is removed; donations are anonymized to preserve financial records. Session is destroyed on deletion.
-   **Donations → Zeffy**: All in-app Stripe donation checkout removed (Apple 3.1.1 compliance). "Donate Now" buttons now link to https://www.zeffy.com/en-US/donation-form/donate-to-change-lives-through-christ (0% platform fees). DonationsPage replaced with a Zeffy link page.
-   **iOS Camera Permissions**: Added `NSCameraUsageDescription`, `NSPhotoLibraryUsageDescription`, `NSPhotoLibraryAddUsageDescription`, and `NSMicrophoneUsageDescription` to `ios/App/App/Info.plist` to fix camera crash on iPad (Apple 2.1(a) compliance).
